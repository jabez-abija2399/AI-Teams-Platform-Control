...
export class DeveloperService {
  public static async generateCode(input: { projectId: string; projectName?: string; visionPrompt: string }): Promise<any> {
    const res = await implementArchitecture(input.projectId, { title: input.projectName || 'App' } as any);
    if (!res.success) {
      throw new Error(res.error.message);
    }
    const changes = res.data.changes || [];
    return {
      generatedFiles: changes.map((c: any) => ({
        path: c.file,
        content: c.code,
        language: getLanguageFromPath(c.file) || undefined,
      })),
      totalLOC: changes.reduce((acc: number, c: any) => acc + (c.code ? c.code.split('\n').length : 0), 0),
      executionSummary: res.data.report?.notes || 'Code generation completed.',
    };
  }

  public static async implement(input: { projectId: string; projectName?: string; visionPrompt: string }): Promise<any> {
    const res = await implementArchitecture(input.projectId, { title: input.projectName || 'App' } as any);
    if (!res.success) {
      throw new Error(res.error.message);
    }

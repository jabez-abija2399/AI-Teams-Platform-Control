  ...
  try {
    ...
    if (docs.length >= 3) {
      ...
      if (architecture && database && api) {
        try {
          const analysis = {
            architecture: JSON.parse(architecture.content),
            database: JSON.parse(database.content),
            api: JSON.parse(api.content),
            decisions: [],
          };
    ...
    const inProgress = await prisma.document
      .findFirst({ where: { projectId: id, type: 'ARCHITECT_IN_PROGRESS' } })
      .catch(() => null);
    ...
    console.error('[API architect-status GET] Error:', err);
    return NextResponse.json({ success: true, data: { exists: false, running: false } });
  }
}
